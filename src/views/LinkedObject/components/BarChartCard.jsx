...
const BarChartCard = (props) => {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      switch(category){
        case "policy":
          response = await fetch(`http://localhost:31965/document/PoliciescountsByDate/start=${endIso}&end=${startIso}`,{method:"GET",headers:{
            'Authorization': `Bearer ${token}`
          }});
          break;
        case "claim":
          response = await fetch(`http://localhost:31965/document/ClaimsCountsByDate/start=${endIso}&end=${startIso}`,{method:"GET",headers:{
            'Authorization': `Bearer ${token}`
          }});
          break;
        case "billing":
          response = await fetch(`http://localhost:31965/document/AccountscountsByDate/start=${endIso}&end=${startIso}?center=${props.center}`,{method:"GET",headers:{
            'Authorization': `Bearer ${token}`
          }});
          break;
        case "Account":
        ...
        case "Policy":
          ...
          break;
        case "Submission":
          response = await fetch(`http://localhost:31965/document/SubbmissionscountsByDate/start=${endIso}&end=${startIso}`,{method:"GET",headers:{
            'Authorization': `Bearer ${token}`
          }});
          break;
      }
